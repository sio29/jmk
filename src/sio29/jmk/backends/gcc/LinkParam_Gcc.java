...
public class LinkParam_Gcc extends LinkParamBase{
	public LinkParam_Gcc(ClParamBase cl_param,ClToolsEnvBase tools_env){
		if(ClOutputType.isExe(cl_param.output_type)){
			exe_filename=cl_param.proj_name;
		}else{
	...
	private static String[] converLinkParam(LinkParamBase link_param,ClToolsEnvBase tools_env){
		...
		if(ClTargetType_Gcc.isX86(link_param.target_type)){
...
System.out.println("link_param.exe_filename_tmp:"+link_param.exe_filename_tmp);
		/*
		if(link_param.exe_filename!=null){
			set.add("-o"+link_param.exe_filename);
		}
		*/
		
		if(link_param.exe_filename_tmp!=null){
			set.add("-o"+link_param.exe_filename_tmp);
		}
